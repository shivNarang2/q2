...
class SyncService:
    ...
    def onMessage(self, data: dict):
        if data['type'] == 'probe':
            return {'type': 'update', 'from': len(self.recorddata), 'data': self.recordata[data['from']:]}
        elif data['type'] == 'record':
            self.recorddata.append(data);

            return None
        #i gave my best to understand the code like 4-5hr but was not able to get logic of test syncing function i got that onmessage if received datatypeof record then it
        #will not return anything and will  store in some list else if its a probe then it will check whether  this is equal to other record data or not and then
        #send data again to device to update it
